...
import { setOpenQuestion, selectOpenQuestion } from "../../../components/feature/videoSlice.js"
import { ScaleLoader } from "react-spinners";
const OpenQuestion = ({ text }: any) => {
  const dispatch = useDispatch()
  const summary = useSelector(selectOpenQuestion)
    ...
    if (truncatedText === "") {
      ...
      for (const chunk of chunks) {
        const response = await openai.chat.completions.create({
          model: "gpt-3.5-turbo",
          messages: [
            { role: "system", content: `You are a proffetional Open Question Generator.` },
            { role: "system", content: `` },
      ...
      dispatch(setOpenQuestion(summaries))
      setLoading(false)
    }
  ...
  useEffect(() => {
    if (!hasEffectRun.current) {
      ...
      const length = 3
      let prompt = `Please create only '3' Open Question of the following text in  language of ${language}. Use a ${tone} tone of voice. Use a ${style} writing style.`;
      //  displayLoadingText();
      if (!summary.length > 0) {
        GPT35Turbo(text)
        setPrompt(prompt)
