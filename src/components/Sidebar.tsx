...
import {selectSpaceId} from "../components/feature/videoSlice.js"
...
const Sidebar = ({ sidebarOpen, setSidebarOpen }: SidebarProps) => {
 ...
 const navigate=useNavigate()
  ...
  return (
    <aside
      ref={sidebar}
      className={`absolute border-r border-r-[whitesmoke] left-0 top-0 z-9999 flex h-screen w-72.5 flex-col overflow-y-hidden bg-white duration-300 ease-linear dark:bg-boxdark lg:static lg:translate-x-0 ${
        sidebarOpen ? 'translate-x-0' : '-translate-x-full'
      }`}
    >
      <div className='h-[65px] drop-shadow-1 border-b border-b-[whitesmoke] w-full p-3'>
          <div  className="px-5 flex flex-row justify-between items-center  h-[50px] -mt-2 bg-white rounded-md shadow-1 ">
                <div className="bg-[#25224A] text-white text-3xl justify-center items-center flex w-[13%] rounded-md px-6 h-full">N</div>
                <div>
                  <p className="text-2xl font-semibold">NoteIt</p>
                </div>
                 </div>
        </div>
      {/* <!-- SIDEBAR HEADER --> */}
      <div className="flex items-center justify-between gap-2 px-6 py-5.5 lg:py-6.5">
        ...
        {/* <NavLink className={"flex item-center flex-row "} to="/"> */}
      ...
      <div className="no-scrollbar relative h-screen -mt-6 flex flex-col overflow-y-auto duration-300 ease-linear">
        {/* <!-- Sidebar Menu --> */}
          ...
          <div>
            ...
            <ul className="mb-6 flex flex-col gap-1.5">
               <li>
                <NavLink
                  to={`/overview/${sid}`}
                  className={`group relative flex items-center gap-2.5 rounded-md py-[10px] px-4 font-medium text-black duration-300 ease-in-out hover:bg-white hover:shadow-[rgba(0,_0,_0,_0.24)_0px_3px_8px] dark:hover:bg-meta-4 ${
                    pathname.includes('overview') &&
                    'bg-white shadow-2 dark:bg-meta-4'
                  }`}
                >
              ...
              <SidebarLinkGroup
                activeCondition={
                  pathname === `/dashbord/${sid}` || pathname.includes('dashboard')||pathname === `/overview/${sid}`||pathname.includes('overview')
                }
              >
                {(handleClick, open) => {
                  return (
              ...
              {/* <li>
                <NavLink
                  to={`/widgets/${sid}`}
                  className={`group relative flex items-center gap-2.5 rounded-md py-3 px-4 font-medium text-black duration-300 ease-in-out hover:bg-white hover:shadow-[rgba(0,_0,_0,_0.24)_0px_3px_8px] dark:hover:bg-meta-4 ${
                    pathname.includes('widgets') &&
                    'bg-white shadow-[rgba(0,_0,_0,_0.24)_0px_3px_8px] dark:bg-meta-4'
                  }`}
                >
                  <AiOutlineCompress size={30} />
                 <p className='text-black'>Collect Widgets</p>
                </NavLink>
              </li> */}
              <li>
                <NavLink
                  to={`/analytics/${sid}`}
                  className={`group relative flex items-center gap-2.5 rounded-md py-[10px] px-4 font-medium text-black duration-300 ease-in-out hover:bg-white hover:shadow-[rgba(0,_0,_0,_0.24)_0px_3px_8px] dark:hover:bg-meta-4 ${
                    pathname.includes('analytics') && 'bg-white shadow-2 dark:bg-meta-4'
                  }`}
                >
                  <CiWavePulse1 size={32}/>
                  Analytics
                </NavLink>
              </li>
              <li>
                <NavLink
                  to={`/customization/${sid}`}
                  className={`group relative flex items-center gap-2.5 rounded-md py-[10px] px-4 font-medium text-black duration-300 ease-in-out hover:bg-white hover:shadow-[rgba(0,_0,_0,_0.24)_0px_3px_8px] dark:hover:bg-meta-4 ${
                    pathname.includes('customization') && 'bg-white shadow-2 dark:bg-meta-4'
                  }`}
                >
                  <LuEdit2 size={20}/>
                  Customization
               ...
               <li>
                <NavLink
                  to="/guidelines"
                  className={`group relative flex items-center gap-2.5 rounded-md py-[10px] px-4 font-medium text-black duration-300 ease-in-out hover:bg-white hover:shadow-[rgba(0,_0,_0,_0.24)_0px_3px_8px] dark:hover:bg-meta-4 ${
                    pathname.includes('guidelines') && 'bg-white shadow-2 dark:bg-meta-4'
                  }`}
                >
                ...
                {(handleClick, open) => {
                  return (
                    <React.Fragment>
                      <NavLink
                        to="#"
                        className={`group relative flex items-center gap-2.5 rounded-sm py-2 px-4 font-medium text-black duration-300 ease-in-out hover:bg-white hover:shadow-[rgba(0,_0,_0,_0.24)_0px_3px_8px] dark:hover:bg-meta-4 ${
                          (pathname === '/forms' ||
                        ...
                        <ul className="mt-4 mb-5.5 flex flex-col gap-2.5 pl-6">
                          ...
                          <li>
                            <NavLink
                              ...
                              className={({ isActive }) =>
                                'group relative flex items-center gap-2.5 rounded-md px-4 font-medium text-black duration-300 ease-in-out hover:bg-white hover:shadow-[rgba(0,_0,_0,_0.24)_0px_3px_8px] hover:text-white ' +
                                (isActive && '!text-white')
                              }
                            >
                              Form Layout
        ...
        {/* <!-- Sidebar Menu --> */}
        <div className='h-[60px] w-fll bg-[white] shadow-4 absolute bottom-0 left-0 right-0'>
        <div ref={trigger}
                onClick={() => setDropdownOpen(!dropdownOpen)}  className="py-3 px-2 flex flex-row justify-between cursor-pointer items-center  rounded-md shadow-1 ">
                <div className="bg-[#25224A] text-white text-3xl  h-[40px] w-[40px] rounded-[50%]  justify-center items-center flex">{email[0]}</div>
                <div className=' flex items-center flex-row'>
                  <p className="text-md font-medium">{email}</p>
                  {
                    dropdownOpen?<SlArrowUp size={14} className="ml-2" color="black"/>:
                  <SlArrowDown size={14} className="ml-2" color="black"/>
        ...
        className={`absolute z-999999 right-0 bottom-14  flex w-72 flex-col rounded-xl border -mt-8 border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark ${
